...
const testTasks: Task[] = [
  { id: 1, title: 'Les leksjon', done: false, description: "dwa" },
  { id: 2, title: 'Møt opp på forelesning', done: false, description: "dawda" },
  { id: 3, title: 'Gjør øving', done: false, description: "dwada2e" },
];
...
describe('Create new task (POST)', () => {
  test('Create new task (200 OK)', (done) => {
    axios.post('/tasks', { title: 'Ny oppgave', description: 'ny desc'}).then((response) => {
      expect(response.status).toEqual(200);
...
describe('Update task (UPDATE)', () => {
  test('Update task (200 OK)', (done) => {
    axios.patch('/tasks/2', {id: 2, title: 'Ny oppgave', description: 'ny desc'}).then((response) => {
      expect(response.status).toEqual(200);
      done();
    });
  });
